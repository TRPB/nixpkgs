...

{
  ...
  config = mkMerge [
    ...
    (mkIf (config.boot.enableContainers) (let
      ...
      portForwardScript = portForward:
        ''
          ...
          echo "${pkgs.socat}/bin/socat ${portForward.protocol}-listen:${toString portForward.hostPort},fork,reuseaddr${portForwardBind portForward} ${portForward.protocol}:$CONTAINER_IP:${toString portForward.containerPort}"
    ...
    in {
      ...
      systemd.services = listToAttrs (filter (x: x.value != null) (        # The generic container template used by imperative containers
